...
window.onload = function() {
	...
	function create () {
	   ...
	   //floor.tileScale.set(.15);
	   //floor.alpha = .18;

		//	Here we add a Sprite to the display list
	   //light = game.add.tileSprite(0, 0, 800, 600, 'floor');
	   //light.tileScale.set(.15);
	   //light.alpha = .5;
	   light = game.add.sprite(0, 0, 'floor');
  ...
  //     // the canvas where we will display the scene
	...
	function update () {
		...
		// mask.moveTo(visibility[0][0],visibility[0][1]);
  //     for(var i=1;i<=visibility.length;i++){
		//  	mask.lineTo(visibility[i%visibility.length][0],visibility[i%visibility.length][1]);
		// }
		// mask.endFill()
